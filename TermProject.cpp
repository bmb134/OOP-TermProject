...
private:
	...
	static constexpr char* enumStrings[6] = { "Tiny", "Small", "Medium", "Large", "Huge", "Gargantuan" };
...
public:
	...
	//Abyssal, Aquan, Auran, Celestial, Common, Deep Speech, Draconic, Druidic, Dwarvish, Elvish, Giant, Gnomish, Goblin, Gnoll, Halfling, Ignan, Infernal, Orc, Primordial, Sylvan, Terran, Undercommon
	...
	Entity(std::string n, bool s, double a, int h, int ac, int l, int ex, int str, int dex, int con, int intel, int wis, int cha, bool mag, bool equ)
	{
		name = n;
		sex = s;
		age = a; 
	...
	std::string attack(Entity& entity)
	{
		...
		//void pickHit(hit, critHit, critMiss, attackType, castAbility, entity);
	}
};
...
int main()
{
	std::ifstream input("Entity.json");
	json j_in;
	input >> j_in;
	...
	std::string n = "Willow";
	bool s = j_in["Willow"]["sex"];
	double a = j_in["Willow"]["age"];
	int h = j_in["Willow"]["health"];
	int ac = j_in["Willow"]["armorClass"];
	int l= j_in["Willow"]["level"];
	int ex = j_in["Willow"]["experience"];
	int str = j_in["Willow"]["strength"];
	int dex = j_in["Willow"]["dexterity"];
	int con = j_in["Willow"]["constitution"];
	int intel = j_in["Willow"]["intelligence"];
	int wis = j_in["Willow"]["wisdom"];
	int cha = j_in["Willow"]["charisma"];
	bool mag = j_in["Willow"]["magic"];
	bool equ = j_in["Willow"]["equipment"];

	Entity e(n, s, a, h, ac, l, ex, str, dex, con, intel, wis, cha, mag, equ);
